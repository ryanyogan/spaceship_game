...
fn spawn_asteroid(
    ...
    // Where does the asteroid spawn at? We do not need
    // a y as we are not spawning 3D asteroids.
    ...
    // Think of this as a lambda or HOC, the closure holds the values
...
fn handle_asteroid_collisions(
    ...
    for (entity, collider) in query.iter() {
        for &collided_entity in collider.colliding_entites.iter() {
            // Did it hit another asteroid?  Ignore it
            if query.get(collided_entity).is_ok() {
                continue;
            }

            // Despawn the asteroid on collision
